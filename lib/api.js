...
const CLAVE_TOKEN_FACTURACION = "facturacionToken";

const leerTokenFacturacion = () => {
  try {
    return typeof window !== "undefined" ? sessionStorage.getItem(CLAVE_TOKEN_FACTURACION) : null;
  } catch {
    return null;
  }
};

const guardarTokenFacturacion = (token) => {
  try {
    sessionStorage.setItem(CLAVE_TOKEN_FACTURACION, token);
  } catch {}
};

export const tieneDesbloqueoFacturacion = () => Boolean(leerTokenFacturacion());

export const bloquearFacturacion = () => {
  try {
    sessionStorage.removeItem(CLAVE_TOKEN_FACTURACION);
  } catch {}
  if (typeof window !== "undefined") {
    window.dispatchEvent(new Event("facturacion-bloqueada"));
  }
};

const fetchFacturacion = async (url, options = {}) => {
  const res = await fetch(url, {
    ...options,
    headers: { ...options.headers, "X-Facturacion-Token": leerTokenFacturacion() || "" },
  });

  const renovado = res.headers.get("X-Facturacion-Token");
  if (renovado) guardarTokenFacturacion(renovado);

  if (res.status === 403) {
    const data = await res.clone().json().catch(() => ({}));
    if (data.codigo === "FACTURACION_BLOQUEADA") bloquearFacturacion();
  }
  return res;
};

export const desbloquearFacturacionApi = async (token, codigo) => {
  const res = await fetch(`${API_URL}/facturacion/desbloquear`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.mensaje || "No se pudo desbloquear facturacion");
  guardarTokenFacturacion(data.token);
};

export const obtenerConfigFacturacion = async (token) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/configuracion`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  const data = await res.json();
...
export const guardarDatosEmpresaFacturacion = async (token, datos) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/configuracion/empresa`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const guardarCredencialesSolApi = async (token, usuarioSol, claveSol) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/configuracion/credenciales-sol`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const subirCertificadoFacturacion = async (token, archivo, claveCertificado) => {
  ...
  const res = await fetchFacturacion(`${API_URL}/facturacion/configuracion/certificado`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}` },
    body: formData,
  });
...
export const buscarDocumentoFacturacion = async (token, tipo, numero) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/documento/${tipo}/${numero}`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  const data = await res.json();
...
export const guardarClienteApi = async (token, documento, datos) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/clientes/${documento}`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const emitirComprobanteApi = async (token, payload) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/emitir`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
export const obtenerComprobantes = async (token) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/comprobantes`, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  const data = await res.json();
...
export const emitirNotaCreditoApi = async (token, comprobanteId, motivoCodigo) => {
  const res = await fetchFacturacion(`${API_URL}/facturacion/notas-credito`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
