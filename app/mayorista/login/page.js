...
export default function MayoristaLoginPage() {
  ...
  const manejarSubmit = async (e) => {
    ...
    try {
      ...
      if (data.usuario.rol !== "mayorista") {
        setError("Esta cuenta no tiene acceso aqui.");
        setCargando(false);
        return;
      }

      guardarSesion(data.token, data.usuario);
      router.push("/mayorista/catalogo");
    } catch (err) {
  ...
  return (
    <div className="max-w-sm mx-auto px-4 py-16">
      <h1 className="text-2xl font-bold mb-6">Acceso mayoristas</h1>
