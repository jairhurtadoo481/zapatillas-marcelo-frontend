...
export default function ConfiguracionPage() {
  ...
  if (cargando) {
    return <p className="text-center py-16 text-gray-500">Cargando...</p>;
  }

  return (
    <ProtegerAdmin>
      <div className="max-w-lg mx-auto px-4 py-10">
        <h1 className="text-2xl font-bold mb-6">Configuracion de pagos</h1>

        {error && <p className="text-red-600 text-sm mb-4">{error}</p>}

        <div className="border border-gray-200 rounded-lg p-4 mb-4">
          <p className="font-semibold mb-3">QR de Yape</p>
          {config.qrYape && (
        ...
        <div className="border border-gray-200 rounded-lg p-4">
          <p className="font-semibold mb-3">QR de Plin</p>
          {config.qrPlin && (
